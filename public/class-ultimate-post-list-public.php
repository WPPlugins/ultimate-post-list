...
class Ultimate_Post_List_Public {
	...
	public function __construct( $plugin_name, $plugin_slug, $plugin_version ) {
		...
		add_shortcode( UPL_SHORTCODE_NAME, array( $this, 'upl_shortcode_handler' ) );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( self::$plugin_slug, plugin_dir_url( __FILE__ ) . 'js/ultimate-post-list-public.js', array( 'jquery' ), self::$plugin_version, false );
	 ...
	 * @since	1.0.0
	 */
	public function upl_shortcode_handler( $atts ) {
	...
	public static function print_list( $args ) {
		...
		*/
		...
		if ( $args[ 'list_title' ] and ! empty( $list_settings[ 'url_list_title' ] ) ) {
			$args[ 'list_title' ] = sprintf(
				'<a href="%s">%s</a>',
		...
		if ( $r->have_posts()) {
			...
			$html .= "\n";
			if ( $args[ 'list_title' ] ) {
				$html .= $args[ 'before_title' ] . $args[ 'list_title' ] . $args[ 'after_title' ];
			}
		...
		} elseif ( 'show' == $list_settings[ 'visibility_if_empty' ] ) {
	...
	private static function make_css_file () {
		...
		foreach ( $list_ids as $list_id ) {
			...
			// set CSS for post title if any value
			if ( $css_statement ) {
				$css_code .= sprintf(
					"#%s-%d ul li div.upl-post-title {%s }\n",
					self::$list_indicator,
					$list_id,
					$css_statement
				);
