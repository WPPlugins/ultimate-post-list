...
class Ultimate_Post_List_Admin {
	...
	public function add_metaboxes ( $post ) {
		...
		if ( ! class_exists( 'Ultimate_Post_List_Options' ) ) {
			print '<p>Missing class Ultimate_Post_List_Options.</p>';
			return;
		}
	...
	public function display_activation_message () {
		...
		// set whole message
		printf(
			'<div class="updated"><p>%s %s %s</p></div>',
			sprintf( 
	...
	public function print_clone_admin_notice () {
		...
		if ( $post ) {
			...
			printf(
				...
				sprintf(
					__( 'List %s successfully created.', 'ultimate-post-list' ),
					'<em>' . esc_html( $post->post_title ) . '</em>'
				),
				sprintf(
					'<a href="%s">%s</a>',
					get_edit_post_link( $list_id ),
					__( $text )
				)
			);
		} else {
	...
	public function clone_list () {
		...
		if( empty( $_GET[ $this->upl_id_key ] ) ) {
			wp_die( 
				...
				sprintf( '<p>%s</p>', esc_html__( 'Ultimate Post List can not copy a list. No list to duplicate has been supplied.', 'ultimate-post-list' ) ),
				// title in title tag
				esc_html__( $text ),
